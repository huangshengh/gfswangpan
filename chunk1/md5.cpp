...
string md5file(const char* filename){
	...
	if(file == NULL)
		printf(" open file failed, md5");
	string res = md5file(file);
	std::fclose(file);
	return res;
}

vector<string> chunkmd5(string filename,uint64_t total,int num)
{
    std::FILE* file = std::fopen(filename.data(),"rb");
    if(file == NULL)
        printf(" open file failed, md5");
    vector<string> vec;
    uint64_t sixfour = 67108864;
    for(int i=0;i<num;++i) {
        uint64_t size = min(sixfour,total);
        total -= size;
        MD5_CTX c;
        MD5_Init(&c);

        char buff[1048576];
        size_t littlesize = sizeof(buff);
        ...
        while (nleft > 0) {
            if(nleft<1048576)
                littlesize = nleft;
            nread = std::fread(buff, sizeof(char), littlesize, file);
            MD5_Update(&c, buff, nread);
            nleft -= nread;
        }
        MD5_Final(out, &c);
        string res;
